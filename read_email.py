...
if n == 0:
    ...
    if user_ip == 'Y' or user_ip == 'y':  # proceeds only if user input is Y or y
        ...
        if return_code == 'OK':
            for num in messages[0].split():
                i = i + 1
                dummy, data = mail.fetch(num, '(RFC822)')  # collects only the required part and stores it to "data"
                for response_part in data:
                    if isinstance(response_part, tuple):  # checks for isinstance(__o, __t)
                        ...
                        receive = (datetime_obj.strftime("on %A, %B %d, %Y at %I:%M %p CDT"))  # formats the datetime
                        raw_email = data[0][1]
